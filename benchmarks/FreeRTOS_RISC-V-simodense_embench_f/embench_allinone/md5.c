...
#define LOCAL_SCALE_FACTOR_md5 51

/* BEEBS heap is just an array */
#define HEAP_SIZE_md5 2000
...
static char heap_md5[HEAP_SIZE_md5];

static void *heap_ptr_md5 = NULL;
static void *heap_end_md5 = NULL;
static size_t heap_requested_md5 = 0;
...
benchmark_md5 (void)
{
  return benchmark_body_md5 (LOCAL_SCALE_FACTOR_md5 * CPU_MHZ, MSG_SIZE);
}
...
benchmark_body_md5 (int rpt, int len)
{
  ...
  for (j = 0; j < rpt; j++) {
    init_heap_beebs (&heap_ptr_md5, &heap_end_md5, &heap_requested_md5,
    	(void *) heap_md5, HEAP_SIZE_md5);

    uint8_t *msg = malloc_beebs(&heap_ptr_md5, &heap_end_md5, &heap_requested_md5, len);
    for (i = 0; i < len; i++){
      msg[i] = i;
    }
    md5(msg, len);
    free_beebs(&heap_ptr_md5, &heap_end_md5, &heap_requested_md5, msg);
...
benchmark_md5_t (void* argv)
{
	...
	for (int i=0; i<200; i++ ){
		...
		portENTER_CRITICAL();
		printf("Initialised md5. Warming up %d\n", 1/*WARMUP_HEAT*/);
		portEXIT_CRITICAL();
		
		warm_caches_md5(1);
		
		portENTER_CRITICAL();
		printf("Warmed up md5\n");
		portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) time(); 
		start_insn = (volatile uint64_t) insn(); 
		volatile int result = benchmark_md5 ();
		volatile uint64_t end_time = time();
		volatile uint64_t end_insn = insn();
		
		portENTER_CRITICAL();
		printf ("md5 cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn);
		printf("[OK?%d]\n", verify_benchmark_md5(result));
