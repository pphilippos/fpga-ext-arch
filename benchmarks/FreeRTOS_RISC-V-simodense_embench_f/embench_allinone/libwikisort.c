...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#define LOCAL_SCALE_FACTOR_wikisort 1

static long int seed_wikisort = 0;
...
long
Min_wikisort (const long a, const long b)
{
  if (a < b)
    return a;
  return b;
...
long
Max_wikisort (const long a, const long b)
{
  if (a > b)
    return a;
  return b;
...
bool
TestCompare (Test_wikisort item1, Test_wikisort item2)
{
  return (item1.value < item2.value);
}

typedef bool (*Comparison) (Test_wikisort, Test_wikisort);
...
long
BinaryFirst (const Test_wikisort array[], const long index, const Range range,
...
long
BinaryLast (const Test_wikisort array[], const long index, const Range range,
	    const Comparison compare)
{
  long start = range.start, end = range.end - 1;
  while (start < end)
...
void
InsertionSort (Test_wikisort array[], const Range range, const Comparison compare)
{
  long i;
  for (i = range.start + 1; i < range.end; i++)
    {
      const Test_wikisort temp = array[i];
      long j;
...
void
Reverse (Test_wikisort array[], const Range range)
{
  long index;
  for (index = Range_length (range) / 2 - 1; index >= 0; index--)
    Swap (array[range.start + index], array[range.end - index - 1], Test_wikisort);
}

/* swap a series of values in the array */
void
BlockSwap (Test_wikisort array[], const long start1, const long start2,
	   const long block_size)
{
  long index;
  for (index = 0; index < block_size; index++)
    Swap (array[start1 + index], array[start2 + index], Test_wikisort);
}

/* rotate the values in an array ([0 1 2 3] becomes [1 2 3 0] if we rotate by 1) */
void
Rotate (Test_wikisort array[], const long amount, const Range range, Test_wikisort cache[],
...
void
WikiMerge (Test_wikisort array[], const Range buffer, const Range A, const Range B,
	   const Comparison compare, Test_wikisort cache[], const long cache_size)
{
  /* if A fits into the cache, use that instead of the internal buffer */
  if (Range_length (A) <= cache_size)
    {
      Test_wikisort *A_index = &cache[0];
      Test_wikisort *B_index = &array[B.start];
      Test_wikisort *insert_index = &array[A.start];
      Test_wikisort *A_last = &cache[Range_length (A)];
      Test_wikisort *B_last = &array[B.end];
	  ...
	  while (true)
		  ...
		  (array[B.start + B_count], array[buffer.start + A_count]))
		{
		  Swap (array[A.start + insert],
			array[buffer.start + A_count], Test_wikisort);
		  A_count++;
		  insert++;
		...
		{
		  Swap (array[A.start + insert], array[B.start + B_count], Test_wikisort);
		  B_count++;
		  insert++;
...
void
WikiSort (Test_wikisort array[], const long size, const Comparison compare)
{
...
#define CACHE_SIZE 512
  ...
  Test_wikisort cache[CACHE_SIZE];
  ...
  for (merge_size = 16; merge_size < power_of_two; merge_size += merge_size)
      ...
      while (decimal < size)
	{
	  ...
	  if (compare (array[end - 1], array[start]))
		...
		Swap (array[buffer1.start + index], array[indexA], Test_wikisort);
	      ...
	      blockB =
		MakeRange (B.start,
			   B.start + Min_wikisort (block_size,
					  Range_length (B) -
		  ...
		  if ((Range_length (lastB) > 0
			...
			BinaryFirst (array, minA, lastB, compare);
		      ...
		      Swap (array[blockA.start + 1],
			    array[buffer1.start + indexA++], Test_wikisort);
...
{
  return rand_beebs (&seed_wikisort);
}

long
TestingMostlyDescending (long index, long total)
{
  return total - index + rand_beebs (&seed_wikisort) * 1.0 / RAND_MAX * 5 - 2.5;
}

long
TestingMostlyAscending (long index, long total)
{
  return index + rand_beebs (&seed_wikisort) * 1.0 / RAND_MAX * 5 - 2.5;
}
...
{
  return (rand_beebs (&seed_wikisort) * 1.0 / RAND_MAX <= 0.9) ? index : (index - 2);
}

long
TestingMostlyEqual (long index, long total)
{
  return 1000L + (long) (rand_beebs (&seed_wikisort) % 4);
}


const long max_size_tarfind = 400;
Test_wikisort array1_tarfind[400];
...
verify_benchmark_wikisort (int res __attribute ((unused)))
{
  ...
  return 0 == memcmp (array1_tarfind, exp, max_size_tarfind * sizeof (array1_tarfind[0]));
}
...
{
  int  res = benchmark_body_wikisort (heat);
...
{
  return benchmark_body_wikisort (LOCAL_SCALE_FACTOR_wikisort * CPU_MHZ);
}
	  ...
	  for (index = 0; index < total; index++)
	    {
	      ...
	      array1_tarfind[index] = item;
	    }

	  WikiSort (array1_tarfind, total, compare);
	}
    }
...
void
benchmark_wikisort_t (void* argv)
{
	...
	for (int i=0; i<200; i++ ){
		...
		portENTER_CRITICAL();
		printf("Initialised wikisort. Warming up %d\n", 1/*WARMUP_HEAT*/);
		portEXIT_CRITICAL();
		
		warm_caches_wikisort(1);
		
		portENTER_CRITICAL();
		printf("Warmed up wikisort\n");
		portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) time(); 
		start_insn = (volatile uint64_t) insn(); 
		volatile int result = benchmark_wikisort ();
		volatile uint64_t end_time = time();
		volatile uint64_t end_insn = insn();
		
		portENTER_CRITICAL();
		printf ("wikisort cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn);
		printf("[OK?%d]\n", verify_benchmark_wikisort(result));
