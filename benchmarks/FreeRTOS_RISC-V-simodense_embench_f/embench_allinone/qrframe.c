...
initframe ()
{
  ...
  framebase = calloc_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino,
  					WDB * WD, 1);
  framask = calloc_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino,
  					((WD * (WD + 1) / 2) + 7) / 8, 1);
  rlens = malloc_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino,
  					WD + 1);
  // finders
  putfind ();
  // alignment blocks
...
freeframe ()
{
  free_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino, framebase);
  free_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino, framask);
  free_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino, rlens);
}
...
#include "embench_allinone/ecctable.h"
...
initecc (unsigned char ecc, unsigned char vers)
{
  ...
  if (fsz < 768)		// for ECC math buffers
    fsz = 768;
  qrframe = malloc_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino,
  						fsz);
  ...
  strinbuf = malloc_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino,
  						fsz);
...
freeecc ()
{
  free_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino, qrframe);
  free_beebs (&heap_ptr_qrduino, &heap_end_qrduino, &heap_requested_qrduino, strinbuf);
}
