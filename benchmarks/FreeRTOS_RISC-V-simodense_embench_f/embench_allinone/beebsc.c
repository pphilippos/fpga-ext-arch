...
void
srand_beebs (long int* seed, unsigned int new_seed)
{
...
void
init_heap_beebs (void **heap_ptr, void **heap_end, size_t* heap_requested,
	void *heap, size_t heap_size)
{
  assert(heap_size % sizeof(void *) == 0);  /* see #138 */
  //printf("heap_ptr before %llx after %llx  heap_end before %llx after %llx\n", heap_ptr, heap, heap_end, heap + heap_size); printf ("calls %d\n", ++calls);
  *heap_ptr = (void *) heap;
...
int
check_heap_beebs (void **heap_ptr, void **heap_end, size_t* heap_requested,
	void *heap)
{
...
void *
malloc_beebs (void **heap_ptr, void **heap_end, size_t* heap_requested,
	size_t size)
{
  if (size == 0)
    return NULL;
...
void *
calloc_beebs (void **heap_ptr, void **heap_end, size_t* heap_requested,
	size_t nmemb, size_t size)
{
  void *new_ptr = malloc_beebs (heap_ptr, heap_end, heap_requested, nmemb * size);
...
void *
realloc_beebs (void **heap_ptr, void **heap_end, size_t* heap_requested,
	void *ptr, size_t size)
{
  ...
  void *new_ptr = malloc_beebs (heap_ptr, heap_end, heap_requested, size);
...
void
free_beebs (void **heap_ptr, void **heap_end, size_t* heap_requested,
