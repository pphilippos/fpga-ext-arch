...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#define LOCAL_SCALE_FACTOR_matmult_int 46
...
static int benchmark_body_matmult_int (int  rpt);

static void
warm_caches_matmult_int (int  heat)
{
  int  res = benchmark_body_matmult_int (heat);
...
static int
benchmark_matmult_int (void)
{
  return benchmark_body_matmult_int (LOCAL_SCALE_FACTOR_matmult_int * CPU_MHZ);
}


static int __attribute__ ((noinline))
benchmark_body_matmult_int (int rpt)
{
  int i;
...
RandomInteger (void)
{
  Seed_matmult_int = ((Seed_matmult_int * 133) + 81) % MOD_SIZE;
  return (Seed_matmult_int);
}
...
static int
verify_benchmark_matmult_int (int unused)
{
  int i, j;
  matrix exp = {
...
void
benchmark_matmult_int_t (void* argv)
{
	...
	for (int i=0; i<200; i++ ){
		...
		portENTER_CRITICAL();
		printf("Initialised matmult-int. Warming up %d\n", 1/*WARMUP_HEAT*/);
		portEXIT_CRITICAL();
		
		warm_caches_matmult_int(1);
		
		portENTER_CRITICAL();
		printf("Warmed up matmult-int\n");
		portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) time(); 
		start_insn = (volatile uint64_t) insn(); 
		volatile int result = benchmark_matmult_int ();
		volatile uint64_t end_time = time();
		volatile uint64_t end_insn = insn();
		
		portENTER_CRITICAL();
		printf ("matmult-int cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn);
		printf("[OK?%d]\n", verify_benchmark_matmult_int(result));
