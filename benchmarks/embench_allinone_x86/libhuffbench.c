...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#define LOCAL_SCALE_FACTOR_huffbench 11

/* BEEBS heap is just an array */

#define HEAP_SIZE_huffbench 8192
static char heap_huffbench[HEAP_SIZE_huffbench] __attribute__((aligned));

static void *heap_ptr_huffbench = NULL;
static void *heap_end_huffbench = NULL;
static size_t heap_requested_huffbench = 0;

#define TEST_SIZE_huffbench 500
...
static const byte orig_data_huffbench[TEST_SIZE_huffbench] = {
  'J', '2', 'O', 'Z', 'F', '5', '0', 'F', 'Y', 'L',
  'D', '5', 'U', 'T', 'V', 'Y', 'Y', 'R', 'M', 'T',
  '0', 'V', 'X', 'O', '0', '1', 'V', 'C', '5', 'F',
  'N', 'I', 'B', '1', 'C', 'G', '1', '2', 'M', 'T',
...
static byte test_data_huffbench[TEST_SIZE_huffbench];
...
compdecomp (byte * data, size_t data_len)
{
  ...
  // allocate data space
  byte *comp = (byte *) malloc_beebs (&heap_ptr_huffbench, &heap_end_huffbench, &heap_requested_huffbench,
  	data_len + 1);
  ...
  // remove work areas
  free_beebs (&heap_ptr_huffbench, &heap_end_huffbench, &heap_requested_huffbench, comp);
}
...
{
  return 0 == memcmp (test_data_huffbench, orig_data_huffbench, TEST_SIZE_huffbench * sizeof (orig_data_huffbench[0]));
}
...
{
  int res = benchmark_body_huffbench (heat);
...
{
  return benchmark_body_huffbench (LOCAL_SCALE_FACTOR_huffbench * CPU_MHZ);
}
...
static int __attribute__ ((noinline)) benchmark_body_huffbench (int rpt)
{
  ...
  for (j = 0; j < rpt; j++)
    {
      init_heap_beebs (&heap_ptr_huffbench, &heap_end_huffbench, &heap_requested_huffbench, 
      	(void *) heap_huffbench, HEAP_SIZE_huffbench);

      // initialization
      memcpy (test_data_huffbench, orig_data_huffbench, TEST_SIZE_huffbench * sizeof (orig_data_huffbench[0]));

      // what we're timing
      compdecomp (test_data_huffbench, TEST_SIZE_huffbench);
    }
...
void
benchmark_huffbench_t (void* argv)
{
	...
	for (int i=0; i<200; i++ ){
		...
		//portENTER_CRITICAL();
		printf("Initialised huffbench. Warming up %d\n", 1/*WARMUP_HEAT*/);
		//portEXIT_CRITICAL();
		
		warm_caches_huffbench(1);
		
		//portENTER_CRITICAL();
		printf("Warmed up huffbench\n");
		//portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) 0; 
		start_insn = (volatile uint64_t) 0; 
		volatile int result = benchmark_huffbench ();
		volatile uint64_t end_time = 0;
		volatile uint64_t end_insn = 0;
		
		//portENTER_CRITICAL();
		printf ("huffbench cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn);
		printf("[OK?%d]\n", verify_benchmark_huffbench(result));
