...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#define LOCAL_SCALE_FACTOR_mont64 423
...
/* ------------------------------ main ------------------------------ */
static uint64 in_a_, in_b_, in_m_;

static int benchmark_body_mont64 (int  rpt);

static void
warm_caches_mont64 (int  heat)
{
  int  res = benchmark_body_mont64 (heat);
...
static int
benchmark_mont64 (void)
{
  return benchmark_body_mont64 (LOCAL_SCALE_FACTOR_mont64 * CPU_MHZ);
}


static int __attribute__ ((noinline))
benchmark_body_mont64 (int rpt)
{
  int i;
  int errors;
  ...
  for (i = 0; i < rpt; i++)
    {
      ...
      m = in_m_;			// Must be odd.
      b = in_b_;			// Must be smaller than m.
      a = in_a_;			// Must be smaller than m.
...
static void
initialise_benchmark_mont64 (void)
{
  in_m_ = 0xfae849273928f89fLL;	// Must be odd.
  in_b_ = 0x14736defb9330573LL;	// Must be smaller than m.
  in_a_ = 0x0549372187237fefLL;	// Must be smaller than m.
}

// r is the number of errors therefore if r = 0 then output a 1 for correct

static int
verify_benchmark_mont64 (int r)
{
  return 0 == r;
}

void*
benchmark_mont64_t (void* ar)
{
	uint64_t start_time = 0;
	uint64_t start_insn = 0;
	
	for (int i=0; i<200; i++ ){
		initialise_benchmark_mont64();
		
		//portENTER_CRITICAL();
		printf("Initialised aha-mont64. Warming up %d\n", 1/*WARMUP_HEAT*/);
		//portEXIT_CRITICAL();

		warm_caches_mont64(1);

		//portENTER_CRITICAL();
		printf("Warmed up aha-mont64\n"); fflush(stdout);
		//portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) 0; 
		start_insn = (volatile uint64_t) 0; 
		volatile int result = benchmark_mont64 ();
		volatile uint64_t end_time = 0;
		volatile uint64_t end_insn = 0;
						
		//portENTER_CRITICAL();
		printf ("aha-mont64 cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn); fflush(stdout);
		printf("[OK?%d]\n", verify_benchmark_mont64(result));
		if (i==0) {runs_count+=1;}	if (runs_count==0) return NULL;//{//vTaskEndScheduler();}
		//portEXIT_CRITICAL();	
	}
