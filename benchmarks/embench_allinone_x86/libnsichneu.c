...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#define LOCAL_SCALE_FACTOR_nsichneu 1231
...
{
  int  res = benchmark_body_nsichneu (heat);
...
{
  return benchmark_body_nsichneu (LOCAL_SCALE_FACTOR_nsichneu * CPU_MHZ);
}
...
void
benchmark_nsichneu_t (void* argv)
{
	uint64_t start_time = 0;
	uint64_t start_insn = 0;
	
	for (int i=0; i<200; i++ ){
		initialise_benchmark_nsichneu();
		
		//portENTER_CRITICAL();
		printf("Initialised nsichneu. Warming up %d\n", 1/*WARMUP_HEAT*/);
		//portEXIT_CRITICAL();
		
		warm_caches_nsichneu(1);
		
		//portENTER_CRITICAL();
		printf("Warmed up nsichneu\n");
		//portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) 0; 
		start_insn = (volatile uint64_t) 0; 
		volatile int result = benchmark_nsichneu ();
		volatile uint64_t end_time = 0;
		volatile uint64_t end_insn = 0;
		
		//portENTER_CRITICAL();
		printf ("nsichneu cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn);
		printf("[OK?%d]\n", verify_benchmark_nsichneu(result));
