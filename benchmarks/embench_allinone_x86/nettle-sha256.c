...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#define LOCAL_SCALE_FACTOR_nettle_sha256 475
...
static const uint32_t K_nettle_sha256[64] = {
...
#define COMPRESS(ctx, data) (_nettle_sha256_compress((ctx)->state, (data), K_nettle_sha256))
...
{
  int  res = benchmark_body_nettle_sha256 (heat);
...
{
  return benchmark_body_nettle_sha256 (LOCAL_SCALE_FACTOR_nettle_sha256 * CPU_MHZ);
}
...
void
benchmark_nettle_sha256_t (void* argv)
{
	...
	for (int i=0; i<200; i++ ){
		...
		//portENTER_CRITICAL();
		printf("Initialised nettle-sha256. Warming up %d\n", 1/*WARMUP_HEAT*/);
		//portEXIT_CRITICAL();
		
		warm_caches_nettle_sha256(1);
		
		//portENTER_CRITICAL();
		printf("Warmed up nettle-sha256\n");
		//portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) 0; 
		start_insn = (volatile uint64_t) 0; 
		volatile int result = benchmark_nettle_sha256 ();
		volatile uint64_t end_time = 0;
		volatile uint64_t end_insn = 0;
		
		//portENTER_CRITICAL();
		printf ("nettle-sha256 cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn);
		printf("[OK?%d]\n", verify_benchmark_nettle_sha256(result));
