...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#define LOCAL_SCALE_FACTOR_slre 110
...
char text_slre[] = "abbbababaabccababcacbcbcbabbabcbabcabcbbcbbac";
...
{
  int  res = benchmark_body_slre (heat);
...
benchmark_slre (void)
{
  return benchmark_body_slre (LOCAL_SCALE_FACTOR_slre * CPU_MHZ);
}
...
benchmark_body_slre (int rpt)
{
  ...
  for (j = 0; j < rpt; j++)
    {
      int i;
      int len = strlen (text_slre);
      struct slre_cap captures;
      ret = 0;

      for (i = 0; i < 4; ++i)
	{
	  ret += slre_match (regexes[i], text_slre, len, &captures, 1);
	}
    }
...
benchmark_slre_t (void* argv)
{
	...
	for (int i=0; i<200; i++ ){
		...
		//portENTER_CRITICAL();
		printf("Initialised slre. Warming up %d\n", 1/*WARMUP_HEAT*/);
		//portEXIT_CRITICAL();
		
		warm_caches_slre(1);
		
		//portENTER_CRITICAL();
		printf("Warmed up slre\n");
		//portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) 0; 
		start_insn = (volatile uint64_t) 0; 
		volatile int result = benchmark_slre ();
		volatile uint64_t end_time = 0;
		volatile uint64_t end_insn = 0;
		
		//portENTER_CRITICAL();
		printf ("slre cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn);
		printf("[OK?%d]\n", verify_benchmark_slre(result));
