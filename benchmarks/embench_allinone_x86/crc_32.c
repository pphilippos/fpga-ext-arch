...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#define LOCAL_SCALE_FACTOR_crc32 170
...
crc32pseudo ()
{
  ...
  for (i = 0; i < 1024; ++i)
    {
      oldcrc32 = UPDC32 (rand_beebs (&seed_crc32), oldcrc32);
    }
...
static int benchmark_body_crc32 (int  rpt);

static void
warm_caches_crc32 (int  heat)
{
  int  res = benchmark_body_crc32 (heat);

  return;
}

static int __attribute__ ((noinline))
benchmark_body_crc32 (int rpt)
{
  int i;
  DWORD r;
...
static int
verify_benchmark_crc32 (int r)
{
  return 11433 == r;
}

static int
benchmark_crc32 (void)
{
  return benchmark_body_crc32 (LOCAL_SCALE_FACTOR_crc32 * CPU_MHZ);
}


void
benchmark_crc32_t (void* argv)
{
	...
	for (int i=0; i<200; i++ ){
		...
		//portENTER_CRITICAL();
		printf("Initialised crc32. Warming up %d\n", 1/*WARMUP_HEAT*/);
		//portEXIT_CRITICAL();
		
		warm_caches_crc32(1);
		
		//portENTER_CRITICAL();
		printf("Warmed up crc32\n");
		//portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) 0; 
		start_insn = (volatile uint64_t) 0; 
		volatile int result = benchmark_crc32 ();
		volatile uint64_t end_time = 0;
		volatile uint64_t end_insn = 0;
		
		//portENTER_CRITICAL();
		printf ("crc32 cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn); fflush(stdout);
		printf("[OK?%d]\n", verify_benchmark_crc32(result));
