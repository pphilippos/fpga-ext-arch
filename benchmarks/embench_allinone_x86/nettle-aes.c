...
/* This scale factor will be changed to equalise the runtime of the
   benchmarks. */
#define LOCAL_SCALE_FACTOR_nettle_aes 78
...
#define LEN_nettle_aes 256
unsigned char plaintext[LEN_nettle_aes] =
...
unsigned char expected_nettle_aes[LEN_nettle_aes] =
...
unsigned char encrypted[LEN_nettle_aes];
unsigned char decrypted[LEN_nettle_aes];
...
verify_benchmark_nettle_aes (int res __attribute ((unused)))
{
  ...
  for (unsigned int i = 0; i < LEN_nettle_aes; i++)
    {
      if (encrypted[i] != expected_nettle_aes[i])
	correct = false;
...
{
  int  res = benchmark_body_nettle_aes (heat);
...
{
  return benchmark_body_nettle_aes (LOCAL_SCALE_FACTOR_nettle_aes * CPU_MHZ);
}
  ...
  for (i = 0; i < rpt; i++)
    {
      ...
      aes_encrypt (&encctx, LEN_nettle_aes, encrypted, plaintext);

      aes_set_decrypt_key (&decctx, 32, key);
      aes_decrypt (&decctx, LEN_nettle_aes, decrypted, encrypted);
    }
...
void
benchmark_nettle_aes_t (void* argv)
{
	uint64_t start_time = 0;
	uint64_t start_insn = 0;
	
	for (int i=0; i<200; i++ ){
		initialise_benchmark_nettle_aes();
		
		//portENTER_CRITICAL();
		printf("Initialised nettle-aes. Warming up %d\n", 1/*WARMUP_HEAT*/);
		//portEXIT_CRITICAL();
		
		warm_caches_nettle_aes(1);
		
		//portENTER_CRITICAL();
		printf("Warmed up nettle-aes\n");
		//portEXIT_CRITICAL();
		
		start_time = (volatile uint64_t) 0; 
		start_insn = (volatile uint64_t) 0; 
		volatile int result = benchmark_nettle_aes ();
		volatile uint64_t end_time = 0;
		volatile uint64_t end_insn = 0;
		
		//portENTER_CRITICAL();
		printf ("nettle-aes cycles %llu instructions %llu ", end_time-start_time, end_insn-start_insn);
		printf("[OK?%d]\n", verify_benchmark_nettle_aes(result));
