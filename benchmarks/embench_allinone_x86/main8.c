...
int main ()
{
	...
	pthread_create( &t[4], NULL, benchmark_picojpeg_t, NULL);
	pthread_create( &t[5], NULL, benchmark_primecount_t, NULL);		
	pthread_create( &t[6], NULL, benchmark_qrduino_t, NULL);
	pthread_create( &t[7], NULL, benchmark_sglib_combined_t, NULL);
	for (int i=0; i<8; i++)
		pthread_join( t[i], NULL);
	...
	pthread_create( &t[6], NULL, benchmark_nettle_aes_t, NULL);
	pthread_create( &t[7], NULL, benchmark_nettle_sha256_t, NULL);	
	for (int i=0; i<8; i++)
		pthread_join( t[i], NULL);
    return 0;
}		
